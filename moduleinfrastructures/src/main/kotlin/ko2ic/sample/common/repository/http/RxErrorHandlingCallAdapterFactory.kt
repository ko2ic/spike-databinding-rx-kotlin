            ...
            when (result) {
                is Single<*> -> {
                    result.onErrorResumeNext(Function { throwable ->
                        Single.error(HttpClientBase.asHttpErrorTypeException(throwable))
                    })
                }
                is Observable<*> -> {
                    result.onErrorResumeNext(Function { throwable ->
                        Observable.error(HttpClientBase.asHttpErrorTypeException(throwable))
                    })
                }
                is Completable -> {
                    result.onErrorResumeNext(Function { throwable ->
                        Completable.error(HttpClientBase.asHttpErrorTypeException(throwable))
                    })
                }
            }
            return result
